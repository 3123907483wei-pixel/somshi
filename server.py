import warnings
# 忽略所有用户警告和实验性功能警告，防止它们被打印到 stdout 干扰 MCP 通信
...
async def call_tool(name: str, arguments: dict) -> list[mcp_types.Content]:
   ...
   try:
       # trends() is sync, use asyncio.to_thread to avoid blocking
       result = await asyncio.to_thread(trends_tool.func, **arguments)
